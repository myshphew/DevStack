  ...
  return (
    <div className="h-fit rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-[11px] text-slate-400">
        {techData.length} Technology Selected
      </p>
      <div>
        {techData.map((tech) => (
          <SelectedTechCard
      ...
      <button
        ...
        disabled={techData.length === 0}
        className="mt-8 w-full rounded-md border border-red-200 py-2 text-[11px] font-semibold text-red-500 transition hover:bg-red-50"
      >
        Remove All
      </button>
    </div>
